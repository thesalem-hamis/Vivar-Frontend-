    ...
    <>
      ...
      <header className="fixed top-0 left-0 right-0 z-50 w-full px-6 md:px-12 py-4 transition-all duration-300 pointer-events-none">
        <div className="max-w-[1440px] mx-auto w-full flex items-center justify-between gap-4 pointer-events-auto">
          ...
          >
            <a href="/" className="flex items-center">
              {/* Logo sizing increased for desktop viewports:
                Changed from md:h-16 to md:h-20 lg:h-24 to balance the upscale typography
              */}
              <img
                src={LOGO_MAIN}
                alt="Logo"
                ...
                <div>
                  <button
                    ...
                    className="w-full flex items-center justify-between py-5 text-[#0E292F]/90
                      text-2xl font-bold tracking-tight border-b border-[#0E292F]/10"
                  >
                    <span>Invest with Us</span>
                    <motion.span
                      animate={{ rotate: mobileDropOpen ? 180 : 0 }}
                      transition={{ duration: 0.2 }}
                    >
                      <ChevronDown size={20} className="text-[#0E292F]/60" />
                    </motion.span>
                  </button>
                  <AnimatePresence>
                    {mobileDropOpen && (
                      <motion.div
                        ...
                        exit={{ height: 0, opacity: 0 }}
                        transition={{ duration: 0.25, ease: "easeInOut" }}
                        className="overflow-hidden bg-[#0E292F]/5 rounded-b-[8px]"
                      >
                        {investDropdown.map((item) => (
                          <Link
                            key={item.label}
