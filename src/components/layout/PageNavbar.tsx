...
export default function PageNavbar() {
  ...
  // Lock body scroll when mobile dropdown panel is fully open
  useEffect(() => {
  ...
  return (
    <div ref={containerRef} className="fixed top-4 left-0 right-0 z-50 w-full px-4 font-sans pointer-events-none">
      <div className="max-w-[440px] sm:max-w-[490px] mx-auto relative pointer-events-auto">
        ...
        {/* ── DROPDOWN PANEL WITH SPARK LOGIC FOR INTER-PAGE ROUTES ── */}
        <AnimatePresence>
          {isOpen && (
            <motion.div
                  ...
                  <AnimatePresence>
                    {investOpen && (
                      <motion.div
                        ...
                        exit={{ height: 0, opacity: 0 }}
                        transition={{ duration: 0.22, ease: "easeInOut" }}
                        className="overflow-hidden mx-1.5 mb-1.5 rounded-[10px] bg-neutral-50 border border-neutral-100"
                      >
                        {investDropdown.map((item) => (
                          <Link
                            key={item.label}
                          ...
                          >
                            <div>
                              <p className="text-neutral-800 text-xs font-semibold tracking-tight">
                                {item.label}
                              </p>
                              <p className="text-neutral-500 text-[11px] mt-0.5 font-normal leading-tight">
                                {item.sub}
                              </p>
                            </div>
                            <ArrowUpRight
