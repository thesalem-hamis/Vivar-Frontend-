    ...
    <div className="w-full font-sans mx-auto" ref={containerRef}>
      ...
      <div className="hidden lg:flex items-center bg-white rounded-full p-2 pl-8 w-full max-w-5xl mx-auto shadow-[0_25px_60px_-15px_rgba(29,63,72,0.06)] border-[3px] border-stone-100 relative">
        <div className="grid grid-cols-4 flex-1 items-center divide-x divide-stone-100 text-left pr-4">
          {/* Category Dropdown */}
          <div
            className="relative cursor-pointer pr-4 select-none group"
            onClick={() => toggleDropdown("deal")}
          >
            <span className="block text-[11px] font-bold text-[#1D3F48] tracking-tight mb-0.5">
              Category
            </span>
            <div className="flex items-center justify-between">
              <span
                className={`text-[14px] font-semibold capitalize transition-colors ${dealType ? "text-stone-900 font-bold" : "text-stone-500 group-hover:text-stone-800"}`}
              >
                {dealType || "Select Category"}
              </span>
              <ChevronDown
                size={14}
                className="text-stone-400 group-hover:text-[#1D3F48] transition-colors"
              />
            </div>
            <AnimatePresence>
              {activeDropdown === "deal" && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 15 }}
                  className="absolute left-0 top-[110%] mt-2 z-40 bg-white border border-stone-100 shadow-xl rounded-xl p-1.5 min-w-[160px] space-y-0.5"
                >
                  {["rent", "sale"].map((opt) => (
                    <button
                      key={opt}
                      onClick={() => handleSelect(setDealType, opt)}
                      className={`w-full text-left px-3 py-2 text-[12px] font-medium rounded-lg capitalize transition-colors ${dealType === opt ? "bg-[#1D3F48] text-white" : "text-stone-600 hover:bg-stone-50"}`}
                    >
                      {opt}
                    </button>
                  ))}
          ...
          {/* Location Dropdown */}
          <div
            className="relative cursor-pointer px-6 select-none group"
          ...
          >
            ...
            <div className="flex items-center justify-between">
              <span
                className={`text-[14px] font-semibold capitalize transition-colors ${location ? "text-stone-900 font-bold" : "text-stone-500 group-hover:text-stone-800"}`}
              >
            ...
            <AnimatePresence>
              {activeDropdown === "location" && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: 15 }}
                  className="absolute left-4 top-[110%] mt-2 z-40 bg-white border border-stone-100 shadow-xl rounded-xl p-1.5 min-w-[180px] space-y-0.5"
                >
                  {[
                    "ikoyi",
                    "lekki",
                    ...
                    <button
                      key={opt}
                      onClick={() => handleSelect(setLocation, opt)}
                      className={`w-full text-left px-3 py-2 text-[12px] font-medium rounded-lg capitalize transition-colors ${location === opt ? "bg-[#1D3F48] text-white" : "text-stone-600 hover:bg-stone-50"}`}
                    >
                      {opt}
                    </button>
                  ))}
          ...
          >
            ...
            <div className="flex items-center justify-between">
              <span
                className={`text-[14px] font-semibold capitalize transition-colors ${homeType ? "text-stone-900 font-bold" : "text-stone-500 group-hover:text-stone-800"}`}
              >
              ...
              {activeDropdown === "type" && (
                ...
                >
                  {[
                    ...
                    <button
                      key={opt}
                      onClick={() => handleSelect(setHomeType, opt)}
                      className={`w-full text-left px-3 py-2 text-[12px] font-medium rounded-lg capitalize transition-colors ${homeType === opt ? "bg-[#1D3F48] text-white" : "text-stone-600 hover:bg-stone-50"}`}
                    >
                      {opt}
                    </button>
                  ))}
          ...
          {/* Bedroom Dropdown */}
          <div
            className="relative cursor-pointer pl-6 pr-2 select-none group"
          ...
          >
            ...
            <div className="flex items-center justify-between">
              <span
                className={`text-[14px] font-semibold transition-colors ${beds ? "text-stone-900 font-bold" : "text-stone-500 group-hover:text-stone-800"}`}
              >
            ...
            <AnimatePresence>
              {activeDropdown === "beds" && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: 15 }}
                  className="absolute left-4 top-[110%] mt-2 z-40 bg-white border border-stone-100 shadow-xl rounded-xl p-1.5 min-w-[130px] space-y-0.5"
                >
                  {["2", "3", "4", "5", "6"].map((num) => (
                    <button
                      key={num}
                      onClick={() => handleSelect(setBeds, num)}
                      className={`w-full text-left px-3 py-2 text-[12px] font-medium rounded-lg transition-colors ${beds === num ? "bg-[#1D3F48] text-white" : "text-stone-600 hover:bg-stone-50"}`}
                    >
                      {num} Beds
                    </button>
                  ))}
        ...
        >
          <div className="flex items-center gap-3 text-left min-w-0 flex-1">
            ...
            <div className="flex flex-col min-w-0">
              <span
                className={`text-[12px] capitalize truncate ${!location && !homeType && !dealType && !beds ? "text-stone-500 font-semibold" : "text-stone-900 font-bold"}`}
              >
                {getMobileSummary()}
              </span>
            </div>
      ...
      <AnimatePresence>
        {isMobileFilterOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 lg:hidden bg-stone-900/40 backdrop-blur-sm flex flex-col justify-end font-sans"
          >
            <div
              className="flex-1"
              onClick={() => setIsMobileFilterOpen(false)}
            ...
            >
              ...
              <div className="px-6 py-5 flex items-center justify-between border-b border-stone-100 shrink-0">
                <h3 className="text-[15px] font-bold text-stone-900 text-center w-full relative font-sans">
                  ...
                  <button
                    onClick={() => setIsMobileFilterOpen(false)}
                    className="absolute right-0 top-1/2 -translate-y-1/2 w-7 h-7 flex items-center justify-center rounded-full bg-stone-100 text-stone-600"
                  >
              ...
              <div className="flex-1 overflow-y-auto px-6 py-2 divide-y divide-stone-100">
                ...
                <div className="py-4">
                  ...
                  >
                    <span className="text-[13px] font-bold text-[#1D3F48]">
                      Category
                    </span>
                    <div className="flex items-center gap-1.5">
                      <span
                        className={`text-[13px] capitalize ${dealType ? "text-stone-900 font-bold" : "text-stone-500"}`}
                      >
                        {dealType || "Select Category"}
                      </span>
                      <ChevronDown
                        size={14}
                        className={`text-stone-400 transition-transform ${mobileAccordion === "deal" ? "rotate-180" : ""}`}
                      />
                    </div>
                  </div>
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-2 p-1 bg-stone-50 rounded-xl">
                      {["rent", "sale"].map((opt) => (
                        <button
                          ...
                          className={`h-9 text-[12px] font-semibold rounded-lg capitalize transition-all ${dealType === opt ? "bg-[#3D7188] text-white" : "text-stone-600"}`}
                        >
                          {opt}
                        </button>
                      ))}
                ...
                <div className="py-4">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5">
                      <span
                        className={`text-[13px] capitalize ${location ? "text-stone-900 font-bold" : "text-stone-500"}`}
                      >
                        {location || "Select Location"}
                      </span>
                      <ChevronDown
                        size={14}
                        className={`text-stone-400 transition-transform ${mobileAccordion === "location" ? "rotate-180" : ""}`}
                      />
                    </div>
                  </div>
                    ...
                    <div className="mt-3 space-y-1">
                      {[
                        ...
                        <button
                          ...
                          className={`w-full text-left h-10 px-4 text-[12px] font-semibold rounded-xl capitalize ${location === opt ? "bg-[#3D7188] text-white" : "bg-stone-50 text-stone-600"}`}
                        >
                          {opt}
                        </button>
                      ))}
                ...
                <div className="py-4">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5">
                      <span
                        className={`text-[13px] capitalize ${homeType ? "text-stone-900 font-bold" : "text-stone-500"}`}
                      >
                        {homeType || "Select Type"}
                      </span>
                      <ChevronDown
                        size={14}
                        className={`text-stone-400 transition-transform ${mobileAccordion === "type" ? "rotate-180" : ""}`}
                      />
                    </div>
                  </div>
                    ...
                    <div className="mt-3 space-y-1">
                      {[
                        ...
                        <button
                          ...
                          className={`w-full text-left h-10 px-4 text-[12px] font-semibold rounded-xl capitalize ${homeType === opt ? "bg-[#3D7188] text-white" : "bg-stone-50 text-stone-600"}`}
                        >
                          {opt}
                        </button>
                      ))}
                ...
                <div className="py-4">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5">
                      <span
                        className={`text-[13px] ${beds ? "text-stone-900 font-bold" : "text-stone-500"}`}
                      >
                        {beds ? `${beds} Beds` : "Select Bedrooms"}
                      </span>
                      <ChevronDown
                        size={14}
                        className={`text-stone-400 transition-transform ${mobileAccordion === "beds" ? "rotate-180" : ""}`}
                      />
                    </div>
                  </div>
                    ...
                    <div className="mt-3 grid grid-cols-5 gap-1.5 p-1 bg-stone-50 rounded-xl">
                      {["2", "3", "4", "5", "6"].map((num) => (
                        <button
                          key={num}
                          onClick={() => setBeds(num)}
                          className={`h-9 text-[12px] font-semibold rounded-lg transition-all ${beds === num ? "bg-[#3D7188] text-white" : "text-stone-600"}`}
                        >
                          {num}
                        </button>
                      ))}
