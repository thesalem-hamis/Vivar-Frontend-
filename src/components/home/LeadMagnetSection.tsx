...
export default function LeadMagnetSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitted(true);
      setForm({ name: "", email: "", phone: "" }); // Clear immediately
      setTimeout(() => setSubmitted(false), 3000);
    } catch {
      setSubmitted(true);
      setForm({ name: "", email: "", phone: "" }); // Clear even on error
      setTimeout(() => setSubmitted(false), 3000);
    } finally {
      setSubmitting(false);
    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          >
            ...
            >
              ...
              <motion.button
                type="submit"
                whileHover={submitting || submitted ? {} : { scale: 1.01, backgroundColor: "#ffffff", color: "#030a08" }}
                whileTap={submitting || submitted ? {} : { scale: 0.99 }}
